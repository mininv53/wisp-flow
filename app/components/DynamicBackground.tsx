...
const BG: Record<BgMood,{a1:string;a2:string;acc:string}> = {
  neutral: {a1:'#060608',a2:'#0a0a10',acc:'rgba(255,255,255,.05)'},
  happy:   {a1:'#0d0520',a2:'#120a2e',acc:'rgba(127,119,221,.13)'},
  excited: {a1:'#071a07',a2:'#0d280d',acc:'rgba(90,210,100,.15)'},
  sad:     {a1:'#060a14',a2:'#080c1c',acc:'rgba(70,110,220,.11)'},
  think:   {a1:'#0c0c1a',a2:'#10101e',acc:'rgba(150,130,255,.11)'},
  love:    {a1:'#140810',a2:'#1c0a16',acc:'rgba(210,90,150,.13)'},
  sleepy:  {a1:'#080810',a2:'#0c0c18',acc:'rgba(90,90,150,.09)'},
  rebel:   {a1:'#07100a',a2:'#0a160c',acc:'rgba(90,210,130,.14)'},
  focus:   {a1:'#060e14',a2:'#081218',acc:'rgba(70,150,210,.13)'},
  deep:    {a1:'#08060e',a2:'#0c0812',acc:'rgba(130,70,200,.11)'},
  error:   {a1:'#120606',a2:'#180808',acc:'rgba(210,70,70,.11)'},
  boost:   {a1:'#061008',a2:'#081408',acc:'rgba(110,210,90,.15)'},
  sync:    {a1:'#080c16',a2:'#0a1018',acc:'rgba(110,150,255,.11)'},
  low:     {a1:'#080808',a2:'#0c0c0e',acc:'rgba(110,110,150,.08)'},
  process: {a1:'#0a080e',a2:'#0e0c14',acc:'rgba(150,110,220,.11)'},
  laugh:   {a1:'#0a0e06',a2:'#0e1408',acc:'rgba(170,210,70,.11)'},
}
...
function makeBg(c: {a1:string;a2:string;acc:string}) {
  return `radial-gradient(ellipse at 25% 35%, ${c.acc} 0%, transparent 55%), radial-gradient(ellipse at 75% 65%, ${c.acc} 0%, transparent 50%), linear-gradient(160deg,${c.a1} 0%,${c.a2} 100%)`
}
...
export default function DynamicBackground({mood,intensity=20,keywordParticles=[]}: Props) {
  // Crossfade cu doua layere — cel nou face fade-in peste cel vechi
  const [layerA, setLayerA] = useState(mood)
  const [layerB, setLayerB] = useState(mood)
  ...
  useEffect(()=>{
    ...
    // Anulam timer anterior daca exista
    if(timerRef.current) clearTimeout(timerRef.current)

    if(!showB) {
      // Layer B devine noul mood si face fade-in
      setLayerB(mood)
      setShowB(true)
      // Dupa tranzitie, layer A devine mood nou si ascundem B
      timerRef.current = setTimeout(()=>{
        setLayerA(mood)
        setShowB(false)
      }, 5200)
    } else {
      // Layer A devine noul mood si face fade-in (B dispare)
      setLayerA(mood)
      setShowB(false)
      timerRef.current = setTimeout(()=>{
        setLayerB(mood)
  ...
  const cA = BG[layerA]||BG.neutral
  const cB = BG[layerB]||BG.neutral
  const shapesA = SHAPES[layerA]||SHAPES.neutral
  const shapesB = SHAPES[layerB]||SHAPES.neutral
  const count = Math.floor(10+intensity*.2)

  const itemsA = useRef(Array.from({length:28},(_,i)=>({
  ...
  return (
    <div style={{position:'absolute',inset:0,overflow:'hidden',pointerEvents:'none',zIndex:0}}>
      ...
      {/* Layer A — base */}
      <div style={{position:'absolute',inset:0,background:makeBg(cA),opacity:1}}>
        {itemsA.current.slice(0,count).map(p=>(
          <ShapeEl key={p.id} type={shapesA[p.id%shapesA.length]} size={p.size}
            color={cA.acc.replace(/[\d.]+\)$/,`${p.op})`)}
      ...
      {/* Layer B — crossfade */}
      <div style={{position:'absolute',inset:0,background:makeBg(cB),opacity:showB?1:0,transition:'opacity 5s ease'}}>
        {itemsA.current.slice(0,count).map(p=>(
          <ShapeEl key={p.id} type={shapesB[p.id%shapesB.length]} size={p.size}
            color={cB.acc.replace(/[\d.]+\)$/,`${p.op})`)}
            style={{position:'absolute',left:`${p.x}%`,top:`${p.y}%`,animationName:'bg-drift',animationDuration:`${p.dur+2}s`,animationDelay:`${p.delay+1}s`,animationIterationCount:'infinite',animationTimingFunction:'ease-in-out',['--tx' as any]:`${p.tx}px`,['--ty' as any]:`${p.ty}px`,['--rot' as any]:`${p.rot}deg`,flexShrink:0}}
          />
        ))}
      </div>
