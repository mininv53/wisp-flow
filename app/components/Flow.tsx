...
const MOODS: Record<Mood,{sym:string,label:string,bursts:string[]}> = {
  happy:   { sym:'◎', label:'echilibrat',   bursts:['○','◎','·','∘'] },
  excited: { sym:'◈', label:'energizat',    bursts:['◈','◆','▸','→'] },
  think:   { sym:'◐', label:'reflectez',    bursts:['◐','◑','◒','·'] },
  sleepy:  { sym:'◌', label:'obosit',       bursts:['◌','○','·','◦'] },
  sad:     { sym:'◍', label:'greu',         bursts:['◍','○','·','◦'] },
  laugh:   { sym:'◉', label:'bine',         bursts:['◉','○','◎','·'] },
  love:    { sym:'♡', label:'recunoscător', bursts:['♡','♢','·','○'] },
}
...
// depth level — creste cu cat e mai sincer/deschis
function calcDepthXP(text: string): number {
  const s = text.toLowerCase()
  let bonus = 0
  if (text.length > 100) bonus += 8
  if (text.length > 200) bonus += 5
  if (/simt|cred|mă gândesc|sincer|recunosc|mărturisesc/.test(s)) bonus += 6
  if (/singur|obosit|pierdut|confuz|speriat|îngrijorat/.test(s)) bonus += 5
  if (/relație|dragoste|sex|alcool|fumat|drog/.test(s)) bonus += 4
  return Math.min(bonus, 18)
}

function DepthBar({ level }: { level: number }) {
  const labels = ['distant','deschis','sincer','profund']
...
function TypingMsg({ text, speed = 55 }: { text: string; speed?: number }) {
  ...
  return <>{displayed}<span style={{ opacity: displayed.length < text.length ? 1 : 0, animation: 'cursor-blink .7s infinite' }}>▌</span></>
}
...
export default function Flow({ userId }: { userId?: string }) {
  ...
  const [motivation,setMotivation]=useState<MotivationState>(()=>{
    ...
    const s=localStorage.getItem('flow-motivation')
  ...
  const sendMsg=async(text:string,isVoice=false)=>{
    if(!text.trim()) return
    const dxp=calcDepthXP(text)
    setDepthXP(p=>Math.min(p+dxp,100))
  ...
  return (
    <div style={{minHeight:'100vh',background:'#060608',display:'flex',flexDirection:'column',fontFamily:'"Georgia",serif',position:'relative',overflow:'hidden',color:'rgba(255,255,255,.8)'}}>
      ...
      {/* TASKS — dropdown */}
      {showTasks&&(
        <div style={{margin:'8px 20px',padding:'12px 16px',background:'rgba(255,255,255,.02)',border:'0.5px solid rgba(255,255,255,.05)',borderRadius:12,flexShrink:0,position:'relative',zIndex:3}}>
          {totalTasks>0&&(
            <div style={{height:1,background:'rgba(255,255,255,.05)',marginBottom:10,position:'relative'}}>
              <div style={{position:'absolute',left:0,top:0,height:'100%',background:'rgba(255,255,255,.2)',width:`${progressPct}%`,transition:'width .4s ease'}}/>
            </div>
          )}
          ...
          <div style={{display:'flex',flexDirection:'column',gap:4,maxHeight:100,overflowY:'auto'}}>
            {tasks.map(t=>(
              <div key={t.id} style={{display:'flex',alignItems:'center',gap:8}}>
                <button onClick={()=>toggleTask(t.id)} style={{width:12,height:12,borderRadius:'50%',border:`0.5px solid ${t.done?'rgba(255,255,255,.3)':'rgba(255,255,255,.15)'}`,background:t.done?'rgba(255,255,255,.1)':'transparent',cursor:'pointer',flexShrink:0,fontSize:7,color:'rgba(255,255,255,.5)'}}>
                  {t.done?'✓':''}
                </button>
      ...
      {/* AVATAR */}
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',padding:'12px 0 6px',flexShrink:0,position:'relative',zIndex:2}}>
        <div style={{position:'relative',animation:'float-av 4s ease-in-out infinite'}}>
          ...
          <Avatar mood={mood} speaking={speaking} size={72}/>
          <Burst mood={mood} trigger={burst}/>
        </div>
        {status&&<div style={{marginTop:4,fontSize:9,color:'rgba(255,255,255,.15)',fontFamily:'system-ui'}}>{status}</div>}
      </div>

      <div style={{height:'0.5px',background:'rgba(255,255,255,.04)',margin:'0 20px',flexShrink:0}}/>

      {/* CHAT — bule */}
      <div ref={chatRef} style={{flex:1,overflowY:'auto',padding:'14px 20px',display:'flex',flexDirection:'column',gap:10,position:'relative',zIndex:1}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:'flex',flexDirection:'column',alignItems:m.role==='bot'?'flex-start':'flex-end',animation:'msg-in .3s ease-out'}}>
            <div style={{fontSize:9,color:'rgba(255,255,255,.1)',marginBottom:3,fontFamily:'system-ui',letterSpacing:'.04em'}}>
              {m.role==='bot'?`FLOW · ${m.ts}`:m.ts}{m.isVoice?' · 🎤':''}
            </div>
            <div style={{
              maxWidth:'72%',
              padding:'10px 16px',
              ...
              fontSize:14,
              lineHeight:1.75,
              color:m.role==='bot'?'rgba(255,255,255,.75)':'rgba(255,255,255,.6)',
              background:m.role==='bot'?'rgba(255,255,255,.04)':'rgba(255,255,255,.07)',
              border:`0.5px solid rgba(255,255,255,${m.role==='bot'?.06:.09})`,
            ...
            <div style={{fontSize:9,color:'rgba(255,255,255,.08)',marginTop:2,fontFamily:'system-ui'}}>
              {MOODS[m.mood].sym}
            </div>
          </div>
        ))}
      ...
      <div style={{padding:'8px 16px 24px',flexShrink:0,position:'relative',zIndex:3}}>
        <div style={{display:'flex',alignItems:'flex-end',gap:8,background:'rgba(255,255,255,.03)',border:'0.5px solid rgba(255,255,255,.07)',borderRadius:28,padding:'8px 8px 8px 18px',backdropFilter:'blur(12px)'}}>
          <textarea
            ...
            rows={1}
            style={{flex:1,background:'transparent',border:'none',color:'rgba(255,255,255,.55)',fontSize:14,outline:'none',resize:'none',fontFamily:'Georgia,serif',lineHeight:1.5,maxHeight:120,overflowY:'auto',padding:0}}
          />
          <div style={{display:'flex',gap:6,alignItems:'center',flexShrink:0,paddingBottom:2}}>
            ...
            <button onClick={()=>setVoiceOpen(true)} style={{width:32,height:32,borderRadius:'50%',border:'0.5px solid rgba(255,255,255,.08)',background:'transparent',color:'rgba(255,255,255,.15)',fontSize:11,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'system-ui'}}>○</button>
