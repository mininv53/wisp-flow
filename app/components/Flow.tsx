  ...
  const getReply=async(history:Msg[],text:string)=>{
    try{
      const res=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
        ...
        systemContext:`Ești FLOW, companion AI de productivitate pentru adulți. Ești calm, direct, fără presiune. Maxim 2 propoziții. Ajuți cu focus, taskuri, blocaje, energie. Discret și practic. Răspunzi în română.`
      })})
    ...
    <div style={{minHeight:'100vh',background:'#060608',display:'flex',flexDirection:'column',fontFamily:'"Georgia",serif',position:'relative',overflow:'hidden',color:'rgba(255,255,255,.8)'}}>
        ...
        {showTasks&&(
          <>
            <div style={{display:'flex',gap:6,marginBottom:8}}>
              <input value={newTask} onChange={e=>setNewTask(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addTask()} placeholder="Adaugă task…" style={{flex:1,background:'transparent',border:'none',borderBottom:'0.5px solid rgba(255,255,255,.1)',padding:'4px 0',color:'rgba(255,255,255,.6)',fontSize:13,outline:'none',fontFamily:'Georgia,serif'}}/>
              <button onClick={addTask} style={{background:'none',border:'none',color:'rgba(255,255,255,.25)',fontSize:16,cursor:'pointer',padding:'0 4px'}}>+</button>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:4,maxHeight:140,overflowY:'auto'}}>
              ...
              {tasks.length===0&&<div style={{fontSize:12,color:'rgba(255,255,255,.15)',fontFamily:'system-ui',padding:'4px 0'}}>niciun task</div>}
            </div>
          </>
        )}
      ...
      <div ref={chatRef} style={{flex:1,overflowY:'auto',padding:'12px 20px',display:'flex',flexDirection:'column',gap:10,position:'relative',zIndex:1}}>
        ...
        {isTyping&&(
          <div style={{animation:'msg-in .3s ease-out'}}>
            <div style={{fontSize:10,color:'rgba(255,255,255,.12)',marginBottom:3,fontFamily:'system-ui'}}>FLOW</div>
            <div style={{display:'flex',gap:3}}>{[0,.2,.4].map((d,i)=><span key={i} style={{width:4,height:4,borderRadius:'50%',background:'rgba(255,255,255,.18)',display:'inline-block',animation:`tdot 1.2s ${d}s infinite`}}/>)}</div>
          </div>
        )}
      </div>
      ...
      <div style={{padding:'8px 20px 20px',position:'relative',zIndex:2}}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&input.trim()){sendMsg(input);setInput('')}}} placeholder="Scrie sau vorbește…" style={{flex:1,background:'transparent',border:'none',padding:'5px 0',color:'rgba(255,255,255,.55)',fontSize:14,outline:'none',fontFamily:'Georgia,serif'}}/>
          <button onClick={toggleMic} style={{width:30,height:30,borderRadius:'50%',border:`0.5px solid ${isListening?'rgba(224,75,74,.35)':'rgba(255,255,255,.08)'}`,background:isListening?'rgba(224,75,74,.06)':'transparent',color:isListening?'#E24B4A':'rgba(255,255,255,.2)',fontSize:13,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',animation:isListening?'rpulse .6s infinite':undefined}}>🎤</button>
          <button onClick={()=>setVoiceOpen(true)} style={{width:30,height:30,borderRadius:'50%',border:'0.5px solid rgba(255,255,255,.08)',background:'transparent',color:'rgba(255,255,255,.2)',fontSize:11,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'system-ui'}}>○</button>
          <button onClick={()=>{if(input.trim()){sendMsg(input);setInput('')}}} style={{width:30,height:30,borderRadius:'50%',border:'0.5px solid rgba(255,255,255,.1)',background:'transparent',color:'rgba(255,255,255,.3)',fontSize:14,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>↑</button>
        </div>
      </div>
