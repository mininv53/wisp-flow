...
function useTypingText(fullText:string,active:boolean,speed=45){const [d,setD]=useState('');useEffect(()=>{if(!active||!fullText){setD(fullText);return}setD('');let i=0;const id=setInterval(()=>{i++;setD(fullText.slice(0,i));if(i>=fullText.length)clearInterval(id)},speed);return()=>clearInterval(id)},[fullText,active,speed]);return d}
function TypingMsg({text,speed=45}:{text:string;speed?:number}){const d=useTypingText(text,true,speed);return <>{d}<span style={{opacity:d.length<text.length?1:0,animation:'cursor-blink .7s infinite'}}>▌</span></>}
...
function Avatar({mood,speaking,size=100}:{mood:Mood;speaking:boolean;size?:number}){
  ...
  return(<svg viewBox="0 0 90 90" width={size} height={size}><circle cx="45" cy="45" r="42" fill="#CECBF6"/><circle cx="45" cy="51" r="33" fill="#EEEDFE"/><ellipse cx="30" cy="39" rx="6.5" ry={eyeRy} fill="#3C3489"/><ellipse cx="60" cy="39" rx="6.5" ry={eyeRy} fill="#3C3489"/><circle cx="32.5" cy="36.5" r="2.2" fill="white"/><circle cx="62.5" cy="36.5" r="2.2" fill="white"/><ellipse cx="30" cy="28" rx="9" ry="3" fill="#EEEDFE"/><ellipse cx="60" cy="28" rx="9" ry="3" fill="#EEEDFE"/><rect x="22" y="25.5" width="16" height="4" rx="2" fill="#3C3489" transform={bTL}/><rect x="52" y="25.5" width="16" height="4" rx="2" fill="#3C3489" transform={bTR}/>{speaking?<path d={[mouth,'M32 58 Q45 65 58 58','M34 57 Q45 61 56 57'][mf]} fill="none" stroke="#3C3489" strokeWidth="2.8" strokeLinecap="round"/>:<path d={mouth} fill="none" stroke="#3C3489" strokeWidth="2.8" strokeLinecap="round"/>}<circle cx="17" cy="54" r="8" fill="#F4C0D1" opacity="0.5"/><circle cx="73" cy="54" r="8" fill="#F4C0D1" opacity="0.5"/></svg>)
}

function Burst({mood,trigger}:{mood:Mood;trigger:number}){
  const [ps,setPs]=useState<any[]>([])
  useEffect(()=>{if(!trigger)return;const items=Array.from({length:7+Math.floor(Math.random()*4)},(_,i)=>{const a=Math.random()*Math.PI*2,d=45+Math.random()*60;return{id:Date.now()+i,e:MOODS[mood].bursts[Math.floor(Math.random()*MOODS[mood].bursts.length)],tx:Math.cos(a)*d,ty:Math.sin(a)*d,rot:-180+Math.random()*360,delay:Math.random()*250,size:14+Math.random()*12}});setPs(items);setTimeout(()=>setPs([]),2400)},[trigger])
  return(<div style={{position:'absolute',inset:0,pointerEvents:'none',zIndex:50,overflow:'hidden'}}>{ps.map(p=><span key={p.id} style={{position:'absolute',fontSize:p.size,left:'50%',top:'50%',animationName:'burst',animationDuration:'1.8s',animationTimingFunction:'ease-out',animationDelay:`${p.delay}ms`,animationFillMode:'forwards',['--tx' as any]:`${p.tx}px`,['--ty' as any]:`${p.ty}px`,['--rot' as any]:`${p.rot}deg`}}>{p.e}</span>)}</div>)
}
...
export default function Wisp({userId}:{userId?:string}){
  ...
  const wispSpeak=useCallback((text:string,m:Mood,onDone?:()=>void)=>{
    speak(text,'junior',{onStart:()=>{setSpeaking(true);setMood(m);setStatus('vorbesc...')},onEnd:()=>{setSpeaking(false);setMood('happy');setStatus('gata');onDone?.()},onError:()=>{setSpeaking(false);setMood('happy');setStatus('gata');onDone?.()}})
  },[])
  ...
  const sendMsg=async(text:string,isVoice=false)=>{
    ...
    setMsgs(p=>[...p,{role:'user',text,mood:m,ts:now(),isVoice}]);setMood('think');setStatus('mă gândesc...');setIsTyping(true)
  ...
  const sendVoiceMsg=async(text:string)=>{const m=detectMood(text);setMsgs(p=>[...p,{role:'user' as const,text,mood:m,ts:now(),isVoice:true}]);const reply=await getReply(msgs,text);setMsgs(p=>{const next=[...p,{role:'bot' as const,text:reply.text,mood:reply.mood,ts:now(),isVoice:true}];setTypingMsgIdx(next.length-1);return next});setMood(reply.mood);setVoText(reply.text);setBurst(b=>b+1);awardXP(1);setVoiceMode('responding');wispSpeak(reply.text,reply.mood,()=>{setVoiceMode('idle');setVoText('Ce mai vrei să îmi spui? 🌟')})}
  const toggleMic=()=>{if(isListening){try{recRef.current?.stop()}catch(e){};setIsListening(false);setStatus('gata');return}const SR=(window as any).SpeechRecognition||(window as any).webkitSpeechRecognition;if(!SR){alert('Folosește Chrome.');return}const rec=new SR();rec.lang='ro-RO';rec.continuous=false;rec.interimResults=true;rec.onstart=()=>{setIsListening(true);setMood('excited');setStatus('te ascult...')};rec.onresult=(e:any)=>{let t='';for(let i=e.resultIndex;i<e.results.length;i++)t+=e.results[i][0].transcript;setInput(t);if(e.results[e.results.length-1].isFinal){try{rec.stop()}catch(e){};setIsListening(false);setStatus('gata');sendMsg(t)}};rec.onerror=()=>{setIsListening(false);setStatus('gata')};rec.start();recRef.current=rec}
  ...
  return(
    <div style={{height:'100vh',background:'#0d0520',display:'flex',flexDirection:'column',fontFamily:'system-ui,sans-serif',position:'relative',overflow:'hidden'}}>
      <style>{`@keyframes burst{0%{opacity:1;transform:translate(-50%,-50%) scale(1) rotate(0deg)}100%{opacity:0;transform:translate(calc(-50% + var(--tx)),calc(-50% + var(--ty))) scale(.3) rotate(var(--rot))}}@keyframes msg-in{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}@keyframes ring{0%{opacity:.6;transform:scale(1)}100%{opacity:0;transform:scale(1.5)}}@keyframes tdot{0%,60%,100%{transform:translateY(0)}30%{transform:translateY(-5px)}}@keyframes rpulse{0%,100%{opacity:1}50%{opacity:.4}}@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}@keyframes cursor-blink{0%,49%{opacity:1}50%,100%{opacity:0}}`}</style>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 18px',flexShrink:0,position:'relative',zIndex:3}}>
        <div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:18}}>✨</span><span style={{fontSize:14,fontWeight:600,color:'#c8c5f5',letterSpacing:'.06em'}}>WISP</span><span style={{fontSize:11,color:'rgba(127,119,221,.5)',background:'rgba(127,119,221,.1)',padding:'2px 8px',borderRadius:10,border:'0.5px solid rgba(127,119,221,.2)'}}>Junior · 6-12 ani</span></div>
        <div style={{display:'flex',gap:8,alignItems:'center'}}><button onClick={()=>setShowXP(v=>!v)} style={{display:'flex',alignItems:'center',gap:4,padding:'4px 10px',borderRadius:12,fontSize:11,border:'0.5px solid rgba(127,119,221,.25)',background:'rgba(127,119,221,.08)',color:'#c8c5f5',cursor:'pointer'}}>⚡ {motivation.xp} XP</button><button onClick={()=>setVoiceOpen(true)} style={{display:'flex',alignItems:'center',gap:6,padding:'6px 14px',borderRadius:20,fontSize:12,border:'0.5px solid rgba(127,119,221,.3)',background:'rgba(127,119,221,.1)',color:'#c8c5f5',cursor:'pointer'}}>🎤 Voice</button></div>
      </div>
      {showXP&&<div style={{padding:'0 18px 10px',position:'relative',zIndex:3}}><XPBar state={motivation} newBadges={newBadges}/></div>}
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',padding:'6px 0 10px',gap:6,flexShrink:0,position:'relative',zIndex:2}}>
        <div style={{position:'relative',animation:'float 3s ease-in-out infinite'}}>
          ...
          <Avatar mood={mood} speaking={speaking} size={90}/><Burst mood={mood} trigger={burst}/>
        </div>
        <div style={{textAlign:'center'}}><div style={{fontSize:13,fontWeight:600,color:'#c8c5f5'}}>{MOODS[mood].emoji} WISP</div><div style={{fontSize:10,color:statusColor,marginTop:2,display:'flex',alignItems:'center',justifyContent:'center',gap:3}}><div style={{width:4,height:4,borderRadius:'50%',background:statusColor,animation:status!=='gata'?'rpulse .7s infinite':undefined}}/>{status}</div></div>
      </div>
      <div ref={chatRef} style={{flex:1,overflowY:'auto',padding:'0 14px',display:'flex',flexDirection:'column',gap:8,position:'relative',zIndex:1,scrollbarWidth:'none'}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:'flex',gap:8,alignItems:'flex-end',maxWidth:'85%',alignSelf:m.role==='user'?'flex-end':'flex-start',flexDirection:m.role==='user'?'row-reverse':'row',animation:'msg-in .3s ease-out'}}>
            ...
            <div>
              <div style={{padding:'9px 13px',borderRadius:16,fontSize:13,lineHeight:1.55,background:m.role==='bot'?'rgba(127,119,221,.13)':'rgba(127,119,221,.22)',color:'rgba(255,255,255,.9)',border:`0.5px solid rgba(127,119,221,${m.role==='bot'?.2:.3})`,borderBottomLeftRadius:m.role==='bot'?3:16,borderBottomRightRadius:m.role==='user'?3:16,fontStyle:m.isVoice?'italic':'normal'}}>
                {m.role==='bot'&&i===typingMsgIdx?<TypingMsg text={m.isVoice?`🎤 ${m.text}`:m.text} speed={45}/>:(m.isVoice?`🎤 ${m.text}`:m.text)}
              </div>
              <div style={{fontSize:10,marginTop:3,color:'rgba(255,255,255,.2)',display:'flex',gap:4,justifyContent:m.role==='user'?'flex-end':'flex-start'}}>{MOODS[m.mood].emoji} {m.ts}{m.isVoice?' · 🎤':''}</div>
            </div>
          </div>
        ))}
        {isTyping&&<div style={{display:'flex',gap:8,alignItems:'center',animation:'msg-in .3s ease-out'}}><div style={{width:24,height:24,borderRadius:'50%',background:'rgba(127,119,221,.2)',border:'0.5px solid rgba(127,119,221,.4)',color:'#c8c5f5',fontSize:11,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center'}}>W</div><div style={{display:'flex',gap:3}}>{[0,.2,.4].map((d,i)=><span key={i} style={{width:7,height:7,borderRadius:'50%',background:'rgba(127,119,221,.5)',display:'inline-block',animation:`tdot 1.2s ${d}s infinite`}}/>)}</div></div>}
      </div>
      <div style={{padding:'8px 14px 16px',display:'flex',gap:8,alignItems:'center',position:'relative',zIndex:3}}>
      ...
      {voiceOpen&&(
        <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,.96)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:20,zIndex:100,animation:'msg-in .4s ease-out'}}>
          ...
          <div style={{position:'relative',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1}}>{[140,180,220].map((sz,i)=><div key={i} style={{position:'absolute',width:sz,height:sz,borderRadius:'50%',border:`${i===2?.5:1}px solid rgba(127,119,221,${speaking?.6:.35})`,animation:speaking?`rpulse ${.5+i*.2}s ease-in-out infinite ${i*.15}s`:`ring ${1.8+i*.3}s ease-out infinite ${i*.3}s`}}/>)}<div style={{position:'relative',zIndex:2,animation:'float 3s ease-in-out infinite'}}><Avatar mood={mood} speaking={speaking} size={130}/><Burst mood={mood} trigger={burst}/></div></div>
          <div style={{textAlign:'center',zIndex:1}}><div style={{fontSize:22,fontWeight:300,letterSpacing:'.08em',color:'#c8c5f5'}}>WISP</div><div style={{fontSize:11,color:'rgba(127,119,221,.4)',marginTop:2}}>{voiceMode==='idle'?'apasă pentru a vorbi':voiceMode==='recording'?'🔴 te ascult...':voiceMode==='analyzing'?'⏳ analizez...':'🔊 răspund...'}</div></div>
          <div style={{maxWidth:260,textAlign:'center',fontSize:14,color:'rgba(255,255,255,.75)',minHeight:52,lineHeight:1.65,fontStyle:voiceMode==='recording'?'italic':'normal',background:'rgba(127,119,221,.08)',borderRadius:16,padding:'12px 20px',border:'0.5px solid rgba(127,119,221,.15)',zIndex:1}}>{voText}</div>
          <button onClick={handleVoBtn} style={{width:74,height:74,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:30,cursor:'pointer',border:`1.5px solid rgba(127,119,221,${voiceMode==='recording'?.3:voiceMode==='responding'?.5:.2})`,background:voiceMode==='recording'?'rgba(224,75,74,.15)':voiceMode==='responding'?'rgba(127,119,221,.2)':'rgba(127,119,221,.1)',animation:voiceMode==='recording'?'rpulse .7s infinite':undefined,transition:'all .25s',zIndex:1}}>{voiceMode==='idle'?'🎤':voiceMode==='recording'?'⏹':voiceMode==='analyzing'?'⏳':'🔊'}</button>
          <div style={{fontSize:11,color:'rgba(255,255,255,.18)',textAlign:'center',maxWidth:220,lineHeight:1.6,zIndex:1}}>{voiceMode==='idle'?'Apasă 🎤, vorbește liber, apoi ⏹ când gata':voiceMode==='recording'?'Apasă ⏹ când ai terminat':voiceMode==='responding'?'Apasă 🔊 pentru a vorbi din nou':''}</div>
        </div>
      )}
