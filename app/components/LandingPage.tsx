...
function HeroAvatar({ mood, speaking, size = 160 }: { mood: string; speaking: boolean; size?: number }) {
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-block' }}>
      {[1, 2, 3].map(i => (
        <div key={i} style={{ position: 'absolute', top: '50%', left: '50%', width: size + i * 32, height: size + i * 32, borderRadius: '50%', border: `0.5px solid rgba(180,100,220,${0.12 - i * 0.03})`, transform: 'translate(-50%,-50%)', animation: `wave-hero ${3 + i * 0.8}s ease-in-out infinite ${i * 0.4}s`, pointerEvents: 'none' }} />
      ))}
...
function ParticleBg({ mx, my }: { mx: number; my: number }) {
  const ps = useRef(Array.from({ length: 40 }, (_, i) => ({
    ...
    type: i % 4, size: 3 + Math.random() * 7,
    dur: 16 + Math.random() * 20, delay: Math.random() * 16,
    tx: (Math.random() - .5) * 100, ty: (Math.random() - .5) * 100,
    warm: i % 3 !== 0, op: 0.06 + Math.random() * 0.04,
  })))
  return (
...
interface LeaderEntry {
  username: string; avatar_initials: string
  product: 'junior' | 'teen' | 'flow'; xp: number; level: number; streak: number
}
const PRODUCT_COLORS: Record<string, string> = { junior: 'rgba(180,130,255,.8)', teen: 'rgba(220,120,60,.8)', flow: 'rgba(160,80,220,.8)' }
const PRODUCT_LABELS: Record<string, string> = { junior: 'Junior', teen: 'Teen', flow: 'Flow' }
...
function LeaderboardSection() {
  ...
  return (
    <section id="leaderboard" className="section-pad" style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 2 }}>
      <div ref={ref} className={`reveal ${visible ? 'in' : ''}`}>
        <div className="leader-header">
          <div>
            ...
            <h2 className="serif" style={{ fontSize: 'clamp(2rem,5vw,4.5rem)', lineHeight: 1.05, color: 'rgba(255,255,255,.88)' }}>
              Cine e pe primul loc<br />
              <span className="serif-i" style={{ color: 'rgba(255,255,255,.25)' }}>azi?</span>
            </h2>
          </div>
          <div style={{ display: 'flex', gap: 4, background: 'rgba(255,255,255,.03)', border: '0.5px solid rgba(255,255,255,.07)', borderRadius: 40, padding: 4, alignSelf: 'flex-start' }}>
            {(['xp', 'streak'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} style={{ padding: '8px 16px', borderRadius: 36, fontSize: 12, fontWeight: 400, cursor: 'pointer', border: 'none', background: tab === t ? (t === 'xp' ? 'rgba(220,120,60,.2)' : 'rgba(160,80,220,.2)') : 'transparent', color: tab === t ? (t === 'xp' ? 'rgba(220,120,60,.9)' : 'rgba(160,80,220,.9)') : 'rgba(255,255,255,.3)', transition: 'all .25s' }}>
                {t === 'xp' ? '⚡ XP' : '🔥 Streak'}
              </button>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          {loading ? Array.from({ length: 5 }).map((_, i) => (
            ...
            <div style={{ textAlign: 'center', padding: '48px 0', color: 'rgba(255,255,255,.2)', fontSize: 14, fontStyle: 'italic' }}>Nimeni încă. Tu poți fi primul. 🚀</div>
          ) : data.map((entry, i) => {
            const isTop3 = i < 3
            const accentCol = PRODUCT_COLORS[entry.product] || 'rgba(220,120,60,.8)'
            return (
              <div key={i} className={`reveal d${Math.min(i + 1, 4)} ${visible ? 'in' : ''}`}
                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 14, background: isTop3 ? 'rgba(255,255,255,.03)' : 'rgba(255,255,255,.015)', border: `0.5px solid ${isTop3 ? 'rgba(220,120,60,.12)' : 'rgba(255,255,255,.04)'}`, transition: 'all .25s', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ width: 28, textAlign: 'center', flexShrink: 0 }}>
                  {medal(i) ? <span style={{ fontSize: 16 }}>{medal(i)}</span> : <span style={{ fontSize: 12, color: 'rgba(255,255,255,.2)', fontFamily: 'monospace' }}>{i + 1}</span>}
                </div>
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: accentCol.replace('.8)', '.15)'), border: `0.5px solid ${accentCol.replace('.8)', '.3)')}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 500, color: accentCol, flexShrink: 0 }}>
                  {entry.avatar_initials || entry.username?.slice(0, 2).toUpperCase() || '??'}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, color: 'rgba(255,255,255,.78)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{entry.username || 'Anonim'}</div>
                  <div style={{ fontSize: 10, color: accentCol.replace('.8)', '.5)'), marginTop: 2, letterSpacing: '.06em' }}>{PRODUCT_LABELS[entry.product]} · Nv. {entry.level}</div>
                </div>
                {tab === 'xp' && (
                  <div style={{ flexShrink: 0, textAlign: 'right' }}>
                    <div style={{ fontSize: 11, color: 'rgba(220,120,60,.7)', fontFamily: 'monospace', fontWeight: 500, marginBottom: 4 }}>{entry.xp.toLocaleString()} XP</div>
                    <div style={{ width: 60, height: 2, background: 'rgba(255,255,255,.06)', borderRadius: 1, overflow: 'hidden' }}>
                ...
                {tab === 'streak' && (
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontSize: 15, color: 'rgba(255,180,60,.8)', fontFamily: 'monospace', fontWeight: 500 }}>🔥 {entry.streak}</div>
        ...
        <div style={{ marginTop: 24, textAlign: 'center', fontSize: 11, color: 'rgba(255,255,255,.14)', fontStyle: 'italic' }}>Live · date anonimizate · fără bs</div>
      </div>
...
export default function LandingPage() {
  ...
  const LINES = ['hey, ce mai faci azi?', 'ai o idee? hai s-o construim.', 'nu trebuie să te forțezi.', 'creierul tău e ok — serios.', 'cu ce începem?']
    ...
    <div style={{ background: '#100a0a', color: 'rgba(255,255,255,.82)', minHeight: '100vh', overflowX: 'hidden', fontFamily: "'DM Sans',system-ui,sans-serif" }}>
      <style>{`
        ...
        *::selection{background:rgba(220,120,60,.3);color:#fff}
        ...
        @keyframes mob-menu-in{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:none}}
        ...
        /* desktop defaults */
        .nav-links{display:flex;align-items:center;gap:32px;font-size:12px;letter-spacing:.12em;text-transform:uppercase}
        .nav-cta{display:block}
        .nav-hamburger{display:none;background:none;border:0.5px solid rgba(255,255,255,.1);border-radius:8px;padding:7px 11px;cursor:pointer;color:rgba(255,255,255,.4);font-size:15px;line-height:1}
        .section-pad{padding:120px 32px}
        .hero-inner{display:flex;align-items:center;gap:60px;min-height:100vh;padding:100px 24px 80px}
        .hero-avatar-col{flex-shrink:0;display:flex;flex-direction:column;align-items:center;gap:24px;position:relative;z-index:1}
        .stats-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:28px;padding-top:28px;border-top:0.5px solid rgba(255,255,255,.06);max-width:480px}
        .grid-3{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:rgba(220,120,60,.06);border-radius:20px;overflow:hidden}
        .grid-3-gap{display:grid;grid-template-columns:repeat(3,1fr);gap:56px}
        .grid-2{display:grid;grid-template-columns:1fr 1fr;gap:40px}
        .grid-2-wide{display:grid;grid-template-columns:1fr 1fr;gap:80px;align-items:start}
        .grid-cta{display:grid;grid-template-columns:1fr auto;gap:60px;align-items:center}
        .leader-header{display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:52px;flex-wrap:wrap;gap:24px}
        .cta-avatar{display:flex;flex-direction:column;align-items:center;gap:12px}
        ...
        @media(max-width:768px){
          ...
          .mobile-menu{
            ...
            border-bottom:0.5px solid rgba(220,120,60,.1);
            padding:16px 24px 20px;
            display:flex;flex-direction:column;gap:2px;
            z-index:49;backdrop-filter:blur(20px);
            animation:mob-menu-in .2s ease-out;
          }
          .mobile-menu a{
          ...
          .mobile-menu-cta{
            ...
            font-size:13px;color:rgba(220,120,60,.8);
            text-decoration:none;text-align:center;
            border:0.5px solid rgba(220,120,60,.25);border-radius:40px;
            background:rgba(220,120,60,.06);
          }
          .section-pad{padding:64px 20px}
          /* hero vertical */
          .hero-inner{flex-direction:column;gap:36px;min-height:auto;padding:96px 20px 60px;text-align:center}
          .hero-text-label{justify-content:center !important}
          .hero-cta-row{justify-content:center !important}
          .stats-grid{max-width:100%;gap:16px}
          .hero-avatar-col{width:100%}
          /* grids single col */
          .grid-3{grid-template-columns:1fr}
          .grid-3-gap{grid-template-columns:1fr;gap:32px}
          .grid-2{grid-template-columns:1fr;gap:24px}
          .grid-2-wide{grid-template-columns:1fr;gap:36px}
          .grid-cta{grid-template-columns:1fr;gap:28px}
          .cta-avatar{display:none}
          .leader-header{flex-direction:column;align-items:flex-start;margin-bottom:28px}
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, transition: 'all .4s', background: scrolled ? 'rgba(16,10,10,.92)' : 'transparent', backdropFilter: scrolled ? 'blur(20px)' : 'none', borderBottom: scrolled ? '0.5px solid rgba(220,120,60,.08)' : 'none' }}>
        ...
        {mobileMenuOpen && (
          <div className="mobile-menu">
            ...
            <Link href="/flow" className="mobile-menu-cta" onClick={() => setMobileMenuOpen(false)}>hai să încercăm →</Link>
          </div>
        )}
      </nav>

      {/* ══ HERO ══ */}
      <section style={{ position: 'relative', maxWidth: 1100, margin: '0 auto', zIndex: 2 }}>
        ...
        <div className="hero-inner">
          <div style={{ flex: 1, position: 'relative', zIndex: 1 }}>
            <div className="hero-text-label" style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 36 }}>
            ...
            <h1 className="serif" style={{ fontSize: 'clamp(2.6rem,7.5vw,6.8rem)', lineHeight: .93, letterSpacing: '-.02em', marginBottom: 28, color: 'rgba(255,255,255,.92)' }}>
              Tu nu ești broken.<br />Sistemul era<br />
              <span className="serif-i" style={{ color: 'rgba(220,120,60,.5)' }}>prost construit.</span>
            </h1>
            <p style={{ fontSize: 15, color: 'rgba(255,255,255,.35)', lineHeight: 1.75, maxWidth: 360, fontWeight: 300, marginBottom: 36 }}>
              Wisp și Flow sunt 3 AI companions — pentru copii, teens și adulți — care se mulează pe creierul tău, nu invers. ADHD sau nu.
            </p>
            <div className="hero-cta-row" style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 48 }}>
              <Link href="/flow" style={{ display: 'flex', alignItems: 'center', gap: 10, background: WARM, color: '#100a0a', fontSize: 14, fontWeight: 500, padding: '13px 26px', borderRadius: 40, textDecoration: 'none', transition: 'all .3s' }}>
            ...
            <div className="stats-grid">
              {[{ n: 3, s: '', label: 'produse · un ecosistem' }, { n: 18, s: '+', label: 'funcții în Flow' }, { n: 10, s: '', label: 'jocuri cognitive' }].map((stat, i) => (
                <div key={i}>
                  <p className="serif" style={{ fontSize: 30, color: 'rgba(255,255,255,.75)', marginBottom: 4 }}><Counter to={stat.n} suffix={stat.s} /></p>
          ...
          <div className="hero-avatar-col">
            ...
            <div style={{ animation: 'breathe-hero 5s ease-in-out infinite', position: 'relative' }}>
              <HeroAvatar mood={avatarMood} speaking={avatarSpeaking} size={160} />
            </div>
            <div style={{ position: 'relative', background: 'rgba(40,20,20,.8)', border: '0.5px solid rgba(220,120,60,.2)', borderRadius: 16, padding: '12px 18px', maxWidth: 200, backdropFilter: 'blur(12px)' }}>
      ...
      <div style={{ borderTop: '0.5px solid rgba(220,120,60,.08)', borderBottom: '0.5px solid rgba(220,120,60,.08)', padding: '14px 0', overflow: 'hidden', position: 'relative', zIndex: 2 }}>
        <div style={{ display: 'flex', gap: 48, whiteSpace: 'nowrap', animation: 'ticker 26s linear infinite' }}>
          {[...Array(2)].map((_, j) => (
            <span key={j} style={{ display: 'flex', gap: 48 }}>
      ...
      {/* ══ PRODUSE ══ */}
      <section id="produse" className="section-pad" style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 2 }}>
        <div ref={s1.ref} className={`reveal ${s1.visible ? 'in' : ''}`} style={{ marginBottom: 48 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            <div style={{ width: 28, height: 1, background: 'rgba(220,120,60,.4)' }} />
            <span style={{ fontSize: 11, color: 'rgba(220,120,60,.5)', letterSpacing: '.2em', textTransform: 'uppercase' }}>Ecosistemul</span>
          </div>
          <h2 className="serif" style={{ fontSize: 'clamp(2rem,5vw,4.5rem)', lineHeight: 1.05, color: 'rgba(255,255,255,.88)' }}>
            Trei produse.<br /><span className="serif-i" style={{ color: 'rgba(255,255,255,.25)' }}>O singură poveste.</span>
          </h2>
        </div>
        <div className="grid-3">
          {[
            ...
            <div key={p.key} ref={s2.ref} className={`reveal d${p.d} ${s2.visible ? 'in' : ''} hover-up`} style={{ background: '#100a0a', padding: 28, display: 'flex', flexDirection: 'column' }}>
              <div style={{ width: 44, height: 44, borderRadius: 14, background: p.dim, border: `0.5px solid ${p.ac.replace('1)', '.2)')}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, marginBottom: 20 }}>{p.icon}</div>
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,.2)', letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 6 }}>{p.badge}</p>
              <h3 className="serif" style={{ fontSize: 24, color: 'rgba(255,255,255,.82)', marginBottom: 10 }}>{p.age}</h3>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,.3)', lineHeight: 1.65, fontWeight: 300, marginBottom: 20, flex: 1 }}>{p.desc}</p>
      ...
      {/* ══ XP ══ */}
      <section id="progres" className="section-pad" style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 2 }}>
        <div ref={s3.ref} className={`reveal ${s3.visible ? 'in' : ''}`}>
          ...
          <h2 className="serif" style={{ fontSize: 'clamp(1.8rem,5vw,4.5rem)', lineHeight: 1.05, color: 'rgba(255,255,255,.88)', marginBottom: 48 }}>
            Nu badge-uri de umplutură.<br /><span className="serif-i" style={{ color: 'rgba(255,255,255,.25)' }}>Ceva ce simți cu adevărat.</span>
          </h2>
          <div className="grid-2">
            <div className={`reveal d1 ${s3.visible ? 'in' : ''}`} style={{ background: 'rgba(220,120,60,.04)', border: '0.5px solid rgba(220,120,60,.1)', borderRadius: 20, padding: 28 }}>
              <div style={{ fontSize: 11, color: 'rgba(220,120,60,.5)', letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 16 }}>Sistemul XP</div>
              <p style={{ fontSize: 20, fontWeight: 300, color: 'rgba(255,255,255,.7)', lineHeight: 1.45, marginBottom: 24 }}>XP mai mare când ești obosit.<br />Pentru că atunci e mai greu.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '0.5px solid rgba(255,255,255,.05)' }}>
                    <div>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'rgba(220,120,60,.9)', fontFamily: 'monospace', flexShrink: 0, marginLeft: 12 }}>{item.xp}</div>
                  </div>
                ))}
              </div>
              <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div className={`reveal d2 ${s3.visible ? 'in' : ''}`} style={{ background: 'rgba(160,80,220,.04)', border: '0.5px solid rgba(160,80,220,.1)', borderRadius: 20, padding: 24 }}>
                <div style={{ fontSize: 11, color: 'rgba(160,80,220,.5)', letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 12 }}>Streak-uri cu cap</div>
                <p style={{ fontSize: 14, color: 'rgba(255,255,255,.55)', lineHeight: 1.55, fontWeight: 300, marginBottom: 12 }}>O zi de grație la 7 zile. Viața e haotică — știm asta.</p>
              ...
              <div className={`reveal d3 ${s3.visible ? 'in' : ''}`} style={{ background: 'rgba(255,255,255,.02)', border: '0.5px solid rgba(255,255,255,.07)', borderRadius: 20, padding: 24 }}>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,.22)', letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 16 }}>Achievements</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
                  {[
                    { icon: '🌱', label: 'Prima sesiune', u: true }, { icon: '🔥', label: '3 zile la rând', u: true },
                    { icon: '⭐', label: '500 XP', u: true }, { icon: '🏆', label: 'O săptămână', u: true },
                    { icon: '💜', label: '1000 XP', u: false }, { icon: '🚀', label: 'O lună', u: false },
                    { icon: '🎯', label: 'Dedicat', u: false }, { icon: '👑', label: 'Maestru', u: false },
                  ].map((b, i) => (
                    <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '8px 4px', borderRadius: 10, background: b.u ? 'rgba(220,120,60,.07)' : 'rgba(255,255,255,.02)', border: `0.5px solid ${b.u ? 'rgba(220,120,60,.2)' : 'rgba(255,255,255,.05)'}`, animationName: s3.visible && b.u ? 'badge-pop' : 'none', animationDuration: '.5s', animationDelay: `${i * .06}s`, animationFillMode: 'both', animationTimingFunction: 'cubic-bezier(.16,1,.3,1)' }}>
                      <span style={{ fontSize: 16, filter: b.u ? 'none' : 'grayscale(1) opacity(.2)' }}>{b.icon}</span>
      ...
      {/* ══ PROBLEMA ══ */}
      <section className="section-pad" style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 2 }}>
        <div ref={s4.ref} className={`reveal ${s4.visible ? 'in' : ''}`}>
          <div className="grid-2-wide">
            <div>
              ...
              <h2 className="serif" style={{ fontSize: 'clamp(1.8rem,4.5vw,3.8rem)', lineHeight: 1.1, color: 'rgba(255,255,255,.88)', marginBottom: 20 }}>
                Alte apps renunță<br /><span className="serif-i" style={{ color: 'rgba(255,255,255,.25)' }}>la tine după<br />11 minute.</span>
              </h2>
      ...
      <section className="section-pad" style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 2 }}>
        <div ref={s5.ref} className={`reveal ${s5.visible ? 'in' : ''}`}>
          ...
          <h2 className="serif" style={{ fontSize: 'clamp(1.8rem,5vw,4.5rem)', lineHeight: 1.05, color: 'rgba(255,255,255,.88)', marginBottom: 56 }}>
            Cu cât vorbești mai mult,<br /><span className="serif-i" style={{ color: 'rgba(255,255,255,.25)' }}>cu atât te cunoaște mai bine.</span>
          </h2>
          <div className="grid-3-gap">
            {[
              ...
              <div key={i} className={`reveal d${i + 1} ${s5.visible ? 'in' : ''}`}>
                <p style={{ fontSize: 11, color: 'rgba(160,80,220,.3)', fontFamily: 'monospace', marginBottom: 16 }}>{s.n}</p>
                <h3 className="serif" style={{ fontSize: 22, color: 'rgba(255,255,255,.78)', marginBottom: 10 }}>{s.title}</h3>
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,.3)', lineHeight: 1.7, fontWeight: 300, marginBottom: 8 }}>{s.body}</p>
      ...
      {/* ══ PRETURI ══ */}
      <section id="preturi" className="section-pad" style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 2 }}>
        <div ref={s6.ref} className={`reveal ${s6.visible ? 'in' : ''}`}>
          ...
          <div className="grid-3">
            {[
              ...
              <div key={i} style={{ background: '#100a0a', padding: 28, display: 'flex', flexDirection: 'column', position: 'relative' }}>
                ...
                <div style={{ marginBottom: 20 }}>
                  <p style={{ fontSize: 10, color: 'rgba(255,255,255,.2)', letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 12 }}>{plan.name}</p>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 4 }}>
                    <span className="serif" style={{ fontSize: 34, color: 'rgba(255,255,255,.78)' }}>{plan.price}</span>
        ...
        <div ref={s7.ref} className={`reveal ${s7.visible ? 'in' : ''}`}>
          <div className="grid-cta">
            <div style={{ maxWidth: 700 }}>
              ...
              <h2 className="serif" style={{ fontSize: 'clamp(2rem,6.5vw,5.5rem)', lineHeight: .95, letterSpacing: '-.02em', color: 'rgba(255,255,255,.9)', marginBottom: 40 }}>
                Copiii cu ADHD<br />nu au nevoie de<br />mai multă disciplină.<br />
                <span className="serif-i" style={{ color: 'rgba(220,120,60,.4)' }}>Au nevoie de cineva<br />care îi înțelege.</span>
              </h2>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                <Link href="/wisp" style={{ display: 'flex', alignItems: 'center', gap: 10, background: WARM, color: '#100a0a', fontSize: 14, fontWeight: 500, padding: '13px 26px', borderRadius: 40, textDecoration: 'none', transition: 'all .3s' }}>
                  Wisp pentru copilul tău <span>→</span>
                </Link>
                {[['Flow pentru tine', '/flow'], ['Wisp Teen', '/wisp-teen']].map(([label, href]) => (
                  <Link key={href} href={href} style={{ fontSize: 13, color: 'rgba(255,255,255,.3)', border: '0.5px solid rgba(255,255,255,.1)', padding: '13px 22px', borderRadius: 40, textDecoration: 'none', transition: 'all .25s' }}>
                    {label}
                  </Link>
                ))}
              </div>
            </div>
            <div className="cta-avatar" style={{ opacity: s7.visible ? 1 : 0, transform: s7.visible ? 'none' : 'translateY(20px)', transition: 'all .9s .4s' }}>
              <div style={{ animation: 'breathe-hero 5s ease-in-out infinite' }}>
                <HeroAvatar mood="happy" speaking={false} size={100} />
              </div>
              <div style={{ fontSize: 10, color: 'rgba(220,120,60,.4)', letterSpacing: '.1em', fontFamily: 'Georgia,serif', fontStyle: 'italic', marginTop: 12, textAlign: 'center' }}>gata când ești tu</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ borderTop: '0.5px solid rgba(220,120,60,.07)', padding: '36px 24px', position: 'relative', zIndex: 2 }}>
